...
body = body.replace(/"enabledTag": true/g, '"enabledTag": false');
body = body.replace(/"point": false/g, '"point": true');
body = body.replace(/"enableLatestPlayableEpisode": false/g, '"enableLatestPlayableEpisode": true');
...
body = body.replace(/"topPrice"\s*:\s*\d+/g, '"topPrice": 0');
body = body.replace(/"floorPrice"\s*:\s*\d+/g, '"floorPrice": 0');
body = body.replace(/"price"\s*:\s*\d+/g, '"price": 0');
body = body.replace(/"auditionEpisode"\s*:\s*\d+/g, '"auditionEpisode": 0');
body = body.replace(/"discountedPrice"\s*:\s*\d+/g, '"discountedPrice": 580');
...
body = body.replace(/"point"\s*:\s*\d+/g, '"point": 9999999');
body = body.replace(/"payPoint" : 0/g, '"payPoint" : 5000');
body = body.replace(/"signStatus" : 0/g, '"signStatus" : 1');
body = body.replace(/"freeAddStudentCount" : 0/g, '"freeAddStudentCount" : 1');
body = body.replace(/"userCoupon"\s*:\s*\d+/g, '"userCoupon": 1');
body = body.replace(/"hasAudition" : 0/g, '"hasAudition" : 1');
