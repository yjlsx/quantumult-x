...
try {
  ...
  if (obj.data && obj.data["{initState}"]) {
    ...
    // 调试：打印关键路径是否存在
    const state = initState.props?.pageProps?.state;
    if (state) {
      logStr += "props.pageProps.state 存在\n";
      if (state.funsionData) {
        logStr += "找到 funsionData\n";
        const funsionData = state.funsionData.data?.data?.get_vip_info_v3?.data;
        if (funsionData) {
          modifyVip(funsionData, "props.pageProps.state.funsionData.data.data.get_vip_info_v3.data");
        } else {
    ...
    const dataVip = initState.initialState?.dataVip?.data;
    if (dataVip) {
      modifyVip(dataVip, "initialState.dataVip.data");
    } else {
