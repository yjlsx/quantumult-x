...
function formalCenter(cookieVal) {
    return new Promise((resolve) => {
        try {
            $.post(formalHost(cookieVal,'lowActive/module/list','{"bizId":29,"configId":1}'), (error, resp, data) => {
                try {
                    let central = JSON.parse(data);
                    $.log("\n----------------------------------------\n\n现在开始正式版任务");
                    if (central.result == 1) {
                        central.modules.forEach(async (lists) => {
  ...
  return new Promise((resolve) =>{
    $.post(formalHost(cookieVal,'task/report', `{"bizId": 29,"taskToken": "${tokens}","eventId": "${eventId}","eventValue": 1}`), (error, resp, data) =>{
      try {
        ...
        if (result.result == 1) {
          let rewards = result.reward.rewardCount;
          let boxname = result.dialog.title;
...
function nebulaInfo(cookieVal) {
   return new Promise((resolve) => {
     $.get(nebulaHost(cookieVal,'activity/earn/overview'), (error, response, data) =>{
       try {
         let result = JSON.parse(data); 
         if (result.result == 1) {
           ...
           for(let nebulaTask of result.data.dailyTasks){
             let taskName = nebulaTask.name
             let taskid = nebulaTask.id
             $.log("去"+taskName)
             if(taskid == 5 && nebulaTask.extParam.todayIsSigned == false){
               nebulaSign(cookieVal);
             } 
             $.log(nebulaTask.desc+"\n");
           }
         } 
...
function nebulaSign(cookieVal) {
   return new Promise((resolve) => {
    $.get(nebulaHost(cookieVal,'sign/sign'), (error, response, data) => {
      try {
        let speed_res = JSON.parse(data);
        let speed_code = speed_res.result;
