...
// 删除指定字段
function removeFields(query, fieldsToRemove) {
...
function rewriteRequest(requestUrl, baseQuery, headers) {
    ...
    // 删除指定字段
    removeFields(currentQuery, ['ignorePreviewAndOthers', 'localDefinition', 'pvod', 'playType']);
    ...
    // 保留 PLANBZP 字段
    if (headers['Cookie'] && headers['Cookie'].includes('PLANBZP')) {
        headers['Cookie'] = headers['Cookie'].split(';').filter(cookie => cookie.trim().startsWith('PLANBZP')).join(';');
    }
